# P07 Masters and inventory enforcement

Phase B | Depends on: P06 | Executor: coordinator (1), worker (2, 3)
Design refs: sections 3.4, 6 in `plans/roles-permissions-entitlements.md`.

## Owned files

Coordinator: `routes/tenant-business.php`, `tenant-stores.php`, `tenant-item-varieties.php`, `tenant-stock-adjustments.php`, `tenant-stock-transfers.php`, `tenant-stock-conversions.php`, `resources/js/lib/nav-items.js` if entries are missing.
Worker: the controllers and Vue pages for these modules (items, customers, suppliers, chart of accounts, stores, item varieties, stock adjustments, transfers, conversions), plus existing tests under
...
Masters (items, parties, chart of accounts, categories) belong to the always-on `core` module, but are still gated by their own permission keys so a role can be restricted. Stores, varieties and stock moves belong to `inventory`.
