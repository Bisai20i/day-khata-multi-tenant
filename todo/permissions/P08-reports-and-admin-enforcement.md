# P08 Reports and admin enforcement

Phase B | Depends on: P07 | Executor: coordinator (1), worker (2, 3)
Design refs: sections 3.4, 6 in `plans/roles-permissions-entitlements.md`.

## Owned files

Coordinator: every `routes/tenant-reports-*.php`, `tenant-activity-log.php`, `tenant-backups.php`, `tenant-notices.php`, `tenant-employees.php`, `tenant-profile.php` (allowlist), `tenant-impersonation.php` (allowlist), `resources/js/lib/nav-items.js` if entries are missing.
Worker: the controllers and Vue pages for these modules (reports, activity log, backups, notices, users), plus existing tests under
...
Reports use one permission per report family plus `.export`/`.print` where ROUTE-MAP splits them. `tenant-employees.php` becomes `can:users.manage`. Backups use owner-only `backups.manage`. Role routes are NOT added here, P11 owns them. After this chunk, run P09 task 3 (route audit).
