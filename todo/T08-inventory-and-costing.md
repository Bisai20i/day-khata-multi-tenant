...
- [x] 1. Migration: `item_stock_movements.value` decimal(15,2) nullable (C10). Backfill best effort:
...
- [x] 2. `Item::recordStockMovement()`, `currentStock()`, `currentStockByItem()`, `lockForStockOut()` exactly
  per C10. `currentStock` via SQL `SUM(CASE ...)` on DECIMAL returning `Quantity`. Casts to `Decimal`.
- [x] 3. `StockCosting` per C10 (weighted average over priced in-movements up to the date, purchase returns
...
- [x] 4. Stock documents (adjustment, transfer, conversion, opening stock import): every posting and cancel
...
- [x] 5. Opening stock import: re-importing replaces the previous opening batch (cancel the prior opening
...
- [x] 6. Items: `ItemController` + `Items/Index.vue` get a posting-account select (Expense or Fixed Asset
...
- [x] 7. Stock pages: previews via `money.js` (`multiplyMoney` per line), 4dp quantities, default date
  `todayInKathmandu()`, list pages with pagination and a date filter (adjustments), BS dates.
- [x] 8. Tests (write, do not run): currentStock exact after 0.1 + 0.2 - 0.3 movements; valuation of a Box
