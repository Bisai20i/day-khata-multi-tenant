...
- [ ] 1. `Sale::post()` on `DocumentCalculator` (C3): VAT rate always from
...
- [ ] 2. PAN invoices (P0-10): `force_non_taxable = true`, voucher type `VoucherType::SalePan`. Reject invoice
...
- [ ] 3. Migration: `sales.fiscal_year_id`, `invoice_number` (unique together), buyer snapshot columns (C7),
...
- [ ] 4. `Sale::outstandingAmount()` returns `Money`: `total - tds_amount - settled_at_posting - sum(posted
...
- [ ] 5. Stock-out safety: inside the transaction call `Item::lockForStockOut()` (C10) before the
  negative-stock check; compare with `Quantity` exactly using `Item::currentStock()`.
- [ ] 6. `Sale::cancel()` per C5: lock, re-check blockers (live returns incl. pending, live receipt
...
- [ ] 7. Controller: request validation `decimal:0,4` for quantity/rate (rate required: a blank rate is a
...
- [ ] 8. `Sales/Create.vue`: all preview math via `calculateDocument()` (C8); show the server-matching totals;
...
- [ ] 9. `Pos.vue`: same calculator; quantities keep 4dp (remove `round2` on quantities); quick-pay fills the
...
- [ ] 10. `Sales/Index.vue`: show `invoice_number` from the server (no hardcoded `SL`/`SLA`), BS date, money
  via `formatMoney`.
- [ ] 11. PDFs (`sale.blade.php`, `sale-receipt.blade.php`): print stored values only; order Subtotal,
...
- [ ] 12. `routes/tenant-sales.php` also holds the capital sale routes (T07 owns the controller): add
...
- [ ] 13. Tests (write, do not run): golden-vector sale posts store exactly the vector values; PAN sale posts
