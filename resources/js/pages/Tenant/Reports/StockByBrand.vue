    ...
    <AppLayout title="Stock by Brand" :nav-items="navItems">
        ...
        <Card variant="panel">
            <div v-if="rows.length === 0" class="px-1 py-6 text-center text-[13px] text-text-muted">
                No stockable item brands found.
            </div>
            ...
            <div v-else class="divide-y divide-border">
                ...
                >
                    ...
                    <div class="w-28 text-right">{{ qty(brand.quantity) }}</div>
                    <div class="w-28 text-right">{{ money(brand.avgCost) }}</div>
                    <div class="w-32 text-right">{{ money(brand.valuation) }}</div>
                </div>
            </div>
