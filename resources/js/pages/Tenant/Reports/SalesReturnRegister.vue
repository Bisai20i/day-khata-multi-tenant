...
const exportUrl = computed(() => {
    ...
    return `/reports/sales-return-register/export?${params.toString()}`;
});
...
const columns = [
    ...
    { id: 'credit_note_number', header: 'Credit Note #', numeric: false, cell: ({ row }) => row.original.credit_note_number ?? '—' },
    { id: 'invoice_number', header: 'Against Invoice #', numeric: false, cell: ({ row }) => row.original.invoice_number ?? '—' },
...
<template>
    <AppLayout title="Sales Return Register" :nav-items="navItems">
        <div class="mb-4 flex items-center justify-between">
            <h2 class="text-base font-bold text-text-strong">Sales Return Register</h2>
        ...
        <p class="mb-4 text-[12.5px] text-text-muted">
            The credit note book. Posted notes only: a pending return request reserves stock but has no money
            or VAT effect, and is never a credit note.
        </p>
        ...
        <Card variant="panel">
            <DataTable :columns="columns" :data="rows" :page-size="25" empty-message="No credit notes in this range" />
