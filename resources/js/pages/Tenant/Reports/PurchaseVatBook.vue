...
const columns = [
    ...
    { id: 'pan_number', header: 'PAN', numeric: false, cell: ({ row }) => row.original.pan_number ?? '—' },
...
<template>
    <AppLayout title="Purchase VAT Book" :nav-items="navItems">
        <div class="mb-4 flex items-center justify-between">
            <h2 class="text-base font-bold text-text-strong">Purchase VAT Book</h2>
        </div>

        <p class="mb-4 text-[12.5px] text-text-muted">
            Posted bills only — this is a tax-filing document, cancelled purchases are excluded.
        </p>
        ...
        <Card variant="panel">
            <DataTable :columns="columns" :data="rows" :page-size="25" empty-message="No posted purchases in this range" />
