...
function submitImport() {
    importForm.post('/suppliers/import', { forceFormData: true });
}
...
function submit() {
    if (editing.value) {
        form.put(`/suppliers/${editing.value.id}`, { onSuccess: closeModal });
    } else {
        form.post('/suppliers', { onSuccess: closeModal });
    }
}
...
<template>
    <AppLayout title="Suppliers" :nav-items="navItems">
        ...
        <Modal :open="importModalOpen" title="Bulk import suppliers" @update:open="onImportModalOpenChange">
            <div v-if="!importResult" class="flex flex-col gap-4">
                <p class="text-[13px] text-text-muted">
                    Download the template, fill in one supplier per row, then upload the completed CSV file. Rows
                ...
                <div>
                    <label for="supplier-import-file" class="mb-1 block text-sm font-semibold text-text-base">
